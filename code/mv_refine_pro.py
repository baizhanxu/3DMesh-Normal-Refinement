...
if ref_vertices.shape[0] < 100:
    print(f"Mesh has too few vertices ({ref_vertices.shape[0]}). Subdividing using pure PyTorch...")
    
    while ref_vertices.shape[0] < 500:
        F = ref_faces.shape[0]
        # 1. 提取所有边 (3条边/面)
...
for i, view_idx in enumerate(view_iter):
    ...
    if M_ref["m00"] > 0 and M_img["m00"] > 0:
        ...
        # Assuming img is already loaded but not resized to 'resolution' yet?
        # Actually img loaded from disk might be different size.
        # But we want to align it to the 'resolution' grid of ref_images.
        # So we align and resize in one go if input is different size?
        # NO, 'img' path logic above loads raw image.
...
with torch.no_grad():
    ...
    # 根据您的要求：为了允许物体的轮廓发生改变，我们不再将细节强行叠加到初始网格（ref_images）的法线上。
    # 我们直接使用上方已经经过 `project_normal_map` 投影到标准法线球面并旋转到世界坐标系的纯 `target_images`。
    ...
    if use_frequency_separation:
        ...
        # # 2. Bilateral Filter (CPU OpenCV implementation for reliability)
        ...
        # # 3. 防止法线向内翻转（Z轴必须大于0，避免变成物体内部反面）
        # # fused_normals[..., 2] 是 Z 轴，表示朝向相机的分量
        # z_clamped = fused_normals[..., 2].clamp(min=1e-3)
        # fused_normals = torch.cat([fused_normals[..., :2], z_clamped.unsqueeze(-1)], dim=-1)
...
opt = MeshOptimizer(vertices,faces, ramp=5, edge_len_lims=(0.005, 0.02), local_edgelen=True, laplacian_weight=0.01) # 0.02,  0.005,0.020
    ...
    if i % 20 == 0:
        with torch.no_grad():
            ...
            for v_idx in range(diff_np.shape[0]):
                ...
                # Make background black where mask is False (eroded or below threshold)
